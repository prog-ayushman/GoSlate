...
function App() {
  ...
  return (
    <>
      ...
      <div className="min-h-200 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 pt-4 pb-0">
        <div className="container  mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="max-w-6xl mx-auto">
            ...
            {/* Date Selector Card */}
            <div className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-3xl p-6 md:p-8 shadow-2xl mb-8 border border-slate-600/50 hover:shadow-cyan-500/10 transition-shadow duration-300">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={selectedDate}
                      onChange={(e) => setSelectedDate(e.target.value)}
                      className="bg-slate-700 border-2 border-slate-600 text-white rounded-xl px-4 py-3 font-semibold focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/30 transition-all duration-300 hover:border-slate-500"
                    />
                  </div>
                </div>

                <div className="text-center md:text-right">
                  <p className="text-sm text-gray-400 font-medium mb-1">Today is</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {[
                { id: 'tasks', label: '📝 Tasks & Goals', color: 'from-cyan-500 to-blue-600' },
                { id: 'meals', label: '🍽️ Meal Planning', color: 'from-emerald-500 to-teal-600' },
                { id: 'classes', label: '📚 Lectures', color: 'from-purple-500 to-pink-600' }
              ].map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`py-4 px-6 rounded-2xl font-bold text-lg transition-all duration-300 transform ${activeTab === tab.id
                    ? `bg-gradient-to-r ${tab.color} text-white shadow-2xl scale-105`
                    : 'bg-slate-800 text-gray-300 shadow-lg border-2 border-slate-700 hover:border-slate-600 hover:shadow-xl'
                    }`}
                >
                  {tab.label}
                </button>
            ...
            {/* Content Container */}
            <div className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-3xl shadow-2xl overflow-hidden border border-slate-600/50">

              {/* Tasks Tab */}
              {activeTab === 'tasks' && (
                <div className="p-8 md:p-12">
                  <h2 className='text-4xl font-bold text-transparent bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text mb-10'>
                    📋 Your Tasks for Today
                  </h2>

                  <div className="space-y-6 mb-10">
                    <div>
                      <label className="text-sm font-semibold text-gray-300 block mb-3">Add a new task</label>
                      <div className="flex flex-col sm:flex-row gap-3">
                        <input
                          ...
                          value={todo}
                          type="text"
                          className='flex-1 bg-slate-700 border-2 border-slate-600 text-white placeholder-gray-500 rounded-xl px-6 py-4 text-lg focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/30 transition-all duration-300 hover:border-slate-500'
                        />
                        <button
                          onClick={handleAdd}
                          disabled={todo.length <= 3}
                          className='bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl hover:from-cyan-600 hover:to-blue-700 disabled:from-slate-600 disabled:to-slate-600 px-8 py-4 text-lg font-bold text-white whitespace-nowrap shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 disabled:transform-none disabled:cursor-not-allowed'
                        >
                          ✨ Add Task
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-3 p-4 bg-slate-700/50 rounded-xl border border-slate-600">
                      <input
                        className='w-5 h-5 rounded focus:ring-2 focus:ring-cyan-500 cursor-pointer'
                        id='show'
                        onChange={toggleFinished}
                        type="checkbox"
                        checked={showFinished}
                      />
                      <label className='font-semibold text-gray-300 cursor-pointer' htmlFor="show">
                        Show Completed Tasks
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    {filteredTodos.length === 0 && (
                      <div className='text-center py-20 bg-slate-700/30 rounded-2xl border-2 border-dashed border-slate-600'>
                        <div className="text-8xl mb-4">🎯</div>
                        <p className="text-2xl font-bold text-gray-300">No tasks yet</p>
                        <p className="text-gray-500 mt-2">Start by adding your first task above!</p>
                      </div>
                    )}
                    {filteredTodos.map(item => {
                      return (showFinished || !item.isCompleted) && (
                        <div
                          key={item.id}
                          className={`flex items-center justify-between p-6 rounded-2xl shadow-md border-2 transition-all duration-300 hover:shadow-lg group ${item.isCompleted
                        ...
                        >
                          <div className='flex items-center gap-4 flex-1'>
                            <input
                              ...
                              type="checkbox"
                              checked={item.isCompleted}
                              className="w-6 h-6 cursor-pointer rounded focus:ring-2 focus:ring-cyan-500"
                            />
                            <div className={`flex-1 text-lg ${item.isCompleted
                              ? "line-through text-gray-500 font-medium"
                            ...
                            {item.isCompleted && <span className="text-2xl">✅</span>}
                          </div>
                          <div className="buttons flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={(e) => handleEdit(e, item.id)}
                              className='bg-cyan-600 hover:bg-cyan-700 p-3 text-white rounded-lg transition-all duration-300 shadow-md hover:shadow-lg'
                            >
                              <FaEdit />
                            </button>
                            <button
                              onClick={(e) => handleDelete(e, item.id)}
                              className='bg-red-600 hover:bg-red-700 p-3 text-white rounded-lg transition-all duration-300 shadow-md hover:shadow-lg'
                            >
                              <AiFillDelete />
                            </button>
                          </div>
              ...
              {activeTab === 'meals' && (
                <div className="p-8 md:p-12">
                  <h2 className='text-4xl font-bold text-transparent bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text mb-10'>
                    🍽️ {getDayName(selectedDate)}'s Menu
                  </h2>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                      { emoji: '🌅', label: 'Breakfast', key: 'breakfast', bgColor: 'from-orange-900/40 to-amber-900/40', borderColor: 'border-orange-600/50', titleColor: 'text-orange-400' },
                      { emoji: '☀️', label: 'Lunch', key: 'lunch', bgColor: 'from-yellow-900/40 to-orange-900/40', borderColor: 'border-yellow-600/50', titleColor: 'text-yellow-400' },
                      { emoji: '🍪', label: 'Snacks', key: 'snacks', bgColor: 'from-pink-900/40 to-rose-900/40', borderColor: 'border-pink-600/50', titleColor: 'text-pink-400' },
                      { emoji: '🌙', label: 'Dinner', key: 'dinner', bgColor: 'from-indigo-900/40 to-purple-900/40', borderColor: 'border-indigo-600/50', titleColor: 'text-indigo-400' }
                    ].map(meal => (
                      <div key={meal.key} className={`bg-gradient-to-br ${meal.bgColor} border-2 ${meal.borderColor} rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105`}>
                        <div className="flex items-center gap-3 mb-4">
                          <span className="text-3xl">{meal.emoji}</span>
                          <h3 className={`text-xl font-bold ${meal.titleColor}`}>{meal.label}</h3>
                        </div>
                        <p className="text-gray-300 leading-relaxed text-sm">{currentMeal[meal.key]}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === 'classes' && (
                <div className="p-8 md:p-12 space-y-8">
                  <div>
                    <h2 className='text-4xl font-bold text-transparent bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text mb-10'>
                      📚 {getDayName(selectedDate)}'s Lectures
                    </h2>

                    {!isSunday(selectedDate) ? (
                      <>
                        <div className="space-y-4 mb-10">
                          <label className="text-sm font-semibold text-gray-300 block">Add a new lecture</label>
                          <div className="flex flex-col sm:flex-row gap-3">
                            <input
                              type="text"
                              placeholder="Enter lecture name"
                              ...
                              onChange={(e) => setNewClass(e.target.value)}
                              onKeyPress={(e) => e.key === 'Enter' && handleAddClass()}
                              className='flex-1 bg-slate-700 border-2 border-slate-600 text-white placeholder-gray-500 rounded-xl px-6 py-4 text-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-500/30 transition-all duration-300 hover:border-slate-500'
                            />
                            <button
                              disabled={!newClass.trim()}
                              onClick={handleAddClass}
                              className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 disabled:from-slate-600 disabled:to-slate-600 text-white px-8 py-4 rounded-xl font-bold text-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 disabled:transform-none disabled:cursor-not-allowed whitespace-nowrap"
                            >
                              ➕ Add Lecture
                            </button>
                          </div>
                        ...
                        <div className="space-y-4">
                          {currentClasses.length > 0
                            ? currentClasses.map((className, idx) => {
                              const attendanceStatus = attendance[selectedDate]?.[className] || "";
                              const statusColors = {
                                "Present": "bg-emerald-900/40 border-emerald-600/50",
                                "Absent": "bg-red-900/40 border-red-600/50",
                                "Off Day": "bg-slate-700/30 border-slate-600",
                              ...
                              return (
                                <div
                                  key={idx}
                                  className={`flex items-center gap-4 p-6 rounded-2xl shadow-md border-2 transition-all duration-300 ${statusColors[attendanceStatus]}`}
                                >
                                  <span className="flex-1 text-lg font-bold text-gray-200">{className}</span>
                                  <select
                                    value={attendanceStatus}
                                    onChange={e => handleAttendance(selectedDate, className, e.target.value)}
                                    className="bg-slate-700 border-2 border-slate-600 text-gray-200 rounded-lg px-4 py-2 font-semibold focus:border-purple-500 focus:ring-2 focus:ring-purple-500/30 transition-all"
                                  >
                            ...
                            : (
                              <div className='text-center py-20 bg-slate-700/30 rounded-2xl border-2 border-dashed border-slate-600'>
                                <div className="text-8xl mb-4">📚</div>
                                <p className="text-2xl font-bold text-gray-300">No lectures yet</p>
                                <p className="text-gray-500 mt-2">Add your first lecture above!</p>
                              </div>
                            )
                          }
                        </div>
                      ...
                      <div className='text-center py-24 bg-gradient-to-br from-purple-900/30 to-pink-900/30 rounded-2xl border-2 border-purple-600/50'>
                        <div className="text-8xl mb-4">🦋</div>
                        <p className="text-3xl font-bold text-purple-300 mb-2">Sunday Off! 🎉</p>
                        <p className="text-lg text-gray-400">Enjoy your well-deserved rest day</p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Attendance Summary */}
                  {Object.keys(attendanceStats).length > 0 && (
                    <div className="border-t-2 border-slate-600 pt-10">
                      <h3 className='text-3xl font-bold text-transparent bg-gradient-to-r from-orange-400 to-red-400 bg-clip-text mb-8'>
                        📊 Attendance Report
                      </h3>

                      <div className="grid grid-cols-1 gap-6">
                        {Object.entries(attendanceStats).map(([className, stats]) => (
                          <div key={className} className="bg-gradient-to-br from-orange-900/30 to-yellow-900/30 rounded-2xl p-6 shadow-md border-2 border-orange-600/50">
                            <div className="flex justify-between items-center mb-6">
                              <h4 className="text-2xl font-bold text-gray-200">{className}</h4>
                              <div className={`text-4xl font-black ${parseFloat(stats.percentage) >= 75 ? 'text-emerald-400' :
                                parseFloat(stats.percentage) >= 50 ? 'text-yellow-400' :
                                  'text-red-400'
                                }`}>
                                {stats.percentage}%
                              </div>
                            ...
                            <div className="grid grid-cols-4 gap-3 mb-4">
                              <div className="bg-emerald-900/40 rounded-lg p-4 text-center border-2 border-emerald-600/50">
                                <div className="text-2xl font-bold text-emerald-400">{stats.present}</div>
                                <div className="text-xs font-bold text-gray-400 mt-1">Present</div>
                              </div>
                              <div className="bg-red-900/40 rounded-lg p-4 text-center border-2 border-red-600/50">
                                <div className="text-2xl font-bold text-red-400">{stats.absent}</div>
                                <div className="text-xs font-bold text-gray-400 mt-1">Absent</div>
                              </div>
                              <div className="bg-slate-700/40 rounded-lg p-4 text-center border-2 border-slate-600">
                                <div className="text-2xl font-bold text-gray-400">{stats.offDay}</div>
                                <div className="text-xs font-bold text-gray-400 mt-1">Off</div>
                              </div>
                              <div className="bg-blue-900/40 rounded-lg p-4 text-center border-2 border-blue-600/50">
                                <div className="text-2xl font-bold text-blue-400">{stats.total}</div>
                                <div className="text-xs font-bold text-gray-400 mt-1">Total</div>
                              </div>
                            </div>

                            <div className="w-full bg-slate-700/50 rounded-full h-3 overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-500 ${parseFloat(stats.percentage) >= 75 ? 'bg-gradient-to-r from-emerald-500 to-emerald-600' :
                                  parseFloat(stats.percentage) >= 50 ? 'bg-gradient-to-r from-yellow-500 to-yellow-600' :
                                    'bg-gradient-to-r from-red-500 to-red-600'
                                  }`}
                                style={{ width: `${stats.percentage}%` }}
                              ></div>
